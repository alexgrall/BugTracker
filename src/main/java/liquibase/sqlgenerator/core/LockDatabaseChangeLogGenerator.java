...
/**
 *  OVERWRITTEN to avoid long time host name initialization
 *  https://liquibase.jira.com/browse/CORE-2938
 */
public class LockDatabaseChangeLogGenerator extends AbstractSqlGenerator<LockDatabaseChangeLogStatement> {
    ...
    private  static final boolean skipHostNameResolving
            = !"false".equalsIgnoreCase(System.getProperty("liquibase.skipHostNameResolving"));

    static {
        try {
            if (!skipHostNameResolving) {
    ...
    public Sql[] generateSql(LockDatabaseChangeLogStatement statement, Database database, SqlGeneratorChain sqlGeneratorChain) {
        ...
        updateStatement.addNewColumnValue("LOCKEDBY", generateHostNameAndDscription());
    ...
    private String generateHostNameAndDscription() {
        StringBuilder hostNameAndDescription = new StringBuilder(hostname);
        if (hostNameAndDescription.length() > 0) {
            hostNameAndDescription.append(' ');
        }
        hostNameAndDescription.append(hostDescription);
        boolean addCyrlyBraces = hostNameAndDescription.length() > 0;
        if (addCyrlyBraces) {
            hostNameAndDescription.append(" (");
        }
        hostNameAndDescription.append(hostaddress);
        if (addCyrlyBraces) {
            hostNameAndDescription.append(')');
        }
        return hostNameAndDescription.toString();
    }
}